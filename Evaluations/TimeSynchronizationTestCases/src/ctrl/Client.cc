...
void Client::handleCycleEvent(cMessage* msg) {
    ...
    if (deviate) {
        ...
        // to avoid scheduling an event to the past
        simtime_t observedNow = readClock();
        simtime_t realNow = simTime();
        simtime_t next =
                (observedNow + cycle) > realNow ?
                        (observedNow + cycle) : realNow;
        scheduleAt(next, cycleTimeout);
    }
