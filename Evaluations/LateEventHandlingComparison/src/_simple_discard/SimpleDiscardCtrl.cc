...
#include "SimpleDiscardCtrl.h"

Define_Module(SimpleDiscardCtrl);

SimpleDiscardCtrl::SimpleDiscardCtrl() :
        lastDelivered(-1, 0, 0) {
...
int SimpleDiscardCtrl::numInitStages() const {
    return STAGE_NUM;
}

void SimpleDiscardCtrl::initialize(int stage) {
    if (stage == 0) {
        HostBase::initialize();
...
void SimpleDiscardCtrl::dispatchHandler(cMessage *msg) {
...
void SimpleDiscardCtrl::handleHeartbeat(HBProbe *hb) {
...
void SimpleDiscardCtrl::handleHeartbeatTimeout(HBTimeout* hbt) {
    UnderlayConfiguratorAccess().get()->revokeNode(ipAddress);
}

void SimpleDiscardCtrl::initNode(simtime_t starttime) {
...
void SimpleDiscardCtrl::handleJoin(InitJoin* initJoin) {
...
void SimpleDiscardCtrl::receiveEvent(Event* event) {
...
void SimpleDiscardCtrl::collectRoundEvents() {
    if (groupId >= 0) {
        round++;
...
void SimpleDiscardCtrl::deliverCycleEvent() {
    ...
    while (round > deliveryRound && mode == EH) {
        ...
        if (Qc.count(deliveryRound) > 0) {
            ...
            for (map<unsigned long, int>::iterator it = senders.begin();
                ...
                tuple_e eKey(deliveryRound, senderId, sor);
                tuple_qp pKey(deliveryRound, senderId, sor);
                if (Qp.count(pKey) > 0) {
                    Events[eKey] = Qp[pKey];
                } else {
        ...
        if (!Delivery.empty()) {
            ...
            set<tuple_qp> QpKeys;
            for (auto elem_qp : Qp) {
                unsigned long senderId_QpKey = get<0>(elem_qp.first);
                int eventRound = get<0>(elem_qp.first);
                if (eventRound <= deliveryRound) {
...
void SimpleDiscardCtrl::handleEvent(CycleEvent* ce) {
// TODO
}

void SimpleDiscardCtrl::handleRoundQuery(RoundQuery* query) {
    ...
    for (map<unsigned long, tuple_seqRange>::iterator it = senderInfos.begin();
        ...
        int sor = get<1>(seqs);
        tuple_r eKey(round, senderId, sor);
        tuple_qp pKey(round, senderId, sor);
        tuple_qd key(round, senderId, sor);
        if (Qd.count(key) > 0) {
...
void SimpleDiscardCtrl::handleRoundQueryReply(RoundQueryReply* rqr) {
...
void SimpleDiscardCtrl::initConsensus() {
...
void SimpleDiscardCtrl::startPropose(ConsensusQuery * cq) {
...
string SimpleDiscardCtrl::roundEToString(int queryRound,
        string senderSeqRanges) {
    string roundEvents;
...
// tuple (round#, sender ID, sequence number, event content)
    ...
    for (map<unsigned long, tuple_seqRange>::iterator it = senderInfos.begin();
        ...
        tuple_seqRange seqs = it->second;
        int sor = get<1>(seqs);
        tuple_r eKey(queryRound, senderId, sor);
        tuple_qp pKey(queryRound, senderId, sor);
        tuple_qd key(queryRound, senderId, sor);
        if (Qd.count(key) > 0) {
            Responses[eKey] = Qd[key];
        } else if (Qp.count(pKey) > 0) {
            Responses[eKey] = Qp[pKey];
...
void SimpleDiscardCtrl::collectProposal(Proposal* proposal) {
...
void SimpleDiscardCtrl::handleConsensusNAck(ConsensusNAck* nack) {
    /*
...
void SimpleDiscardCtrl::checkProposals(int r) {
    if (mode == EH) {
...
map<boost::tuple<int, unsigned long, int>, string> SimpleDiscardCtrl::decide(
        int round, set<string> memberList) {
// tuple(sender ID, lowerest sequence number, sequene of the round)
    typedef boost::tuple<int, int> tuple_seqRange;
    map<unsigned long, tuple_seqRange> senderInfos;
...
// tuple (round#, sender ID, sequence number, event content)
    ...
    for (map<unsigned long, tuple_seqRange>::iterator it = senderInfos.begin();
        ...
        for (set<string>::iterator it = memberList.begin();
            ...
            tuple_ppKey ppKey(senderId, sor);
            string event = events[ppKey];
            boost::tuple<int, unsigned long, int> dKey(round, senderId, sor);
...
void SimpleDiscardCtrl::handleDecision(Decision* decision) {
// validate message
...
void SimpleDiscardCtrl::applyEvent() {
...
void SimpleDiscardCtrl::applyUpdate(UpdateApply* updateApply) {
...
void SimpleDiscardCtrl::disseminateEvent() {
...
void SimpleDiscardCtrl::handleGossip(Gossip* gossip) {
    if (groupId >= 0) {
...
void SimpleDiscardCtrl::stateTransfer() {
    if (groupId >= 0) {
...
bool SimpleDiscardCtrl::checkCoord() {
    string addrStr = ipAddress.get4().str();
    /*
...
string SimpleDiscardCtrl::coordinator() {
...
void SimpleDiscardCtrl::handleCCQuery(CCQuery* cc) {
...
string SimpleDiscardCtrl::buildCCSnapshot() {
    json snapshot;
    ...
    snapshot["deliveryRound"] = deliveryRound;
...
void SimpleDiscardCtrl::handleCCNAck(CCNAck* ccnack) {
...
void SimpleDiscardCtrl::handleCCQueryReply(CCQueryReply* ccr) {
...
void SimpleDiscardCtrl::checkCCSnapshos() {
...
json SimpleDiscardCtrl::computeCCSnapshot() {
    json snapshot;
    ...
    if (reconfig) {
        ...
        for (auto elem : CCSnapshots) {
            ...
            if (!qd_str.empty()) {
                ...
                if (round > lastRound || lastRound < 0) {
                    ...
                    delivery_round = jState["deliveryRound"];
                }
            }
        ...
        init["deliveryRound"] = delivery_round;
...
void SimpleDiscardCtrl::handleCCEnd(CCEnd* cce) {
...
//                    scheduleAt(next_round_time, updateTimeout);
                ...
                deliveryRound = init["deliveryRound"];
...
void SimpleDiscardCtrl::handleCCAck(CCAck* ack) {
...
void SimpleDiscardCtrl::checkCCAcks() {
    bool allReceived = true;
...
bool SimpleDiscardCtrl::checkConfig() {
...
void SimpleDiscardCtrl::handleGRQuery(GRQuery* gr) {
...
string SimpleDiscardCtrl::buildGRSnapshot() {
    json snapshot;
    string Qdstr = util::convertMapToStr(Qd);
    string Qcstr = util::convertMapToStr(Qc);
    snapshot["Qd"] = Qdstr;
...
void SimpleDiscardCtrl::handleGRNAck(GRNAck* grnack) {
...
void SimpleDiscardCtrl::handleGRQueryReply(GRQueryReply* grr) {
    IPvXAddress& srcAddr = check_and_cast<UDPControlInfo *>(
...
void SimpleDiscardCtrl::checkGRSnapshots() {
    if (mode != CC) {
        bool allReceived = true;
...
json SimpleDiscardCtrl::computeGRSnapshot() {
    json snapshot;
    json init;
    string longest_Qd;
    int lastRound = -1;
...
void SimpleDiscardCtrl::handleGREnd(GREnd* gre) {
    IPvXAddress& srcAddr = check_and_cast<UDPControlInfo *>(
...
set<string> SimpleDiscardCtrl::survivedMembers() {
    set<string> survived_members = util::intersectSets(survivals, members);
    return survived_members;
}

IPvXAddress SimpleDiscardCtrl::loadIPAddress() {
...
void SimpleDiscardCtrl::syncClock() {
