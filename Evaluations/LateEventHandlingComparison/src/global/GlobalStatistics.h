...
private:
    ...
    // record the sequence of sent events, in the form of (event_sender_name, logical_computer_name, event_sequence number)
    map<boost::tuple<string, string>, set<boost::tuple<int, simtime_t> > > event_sent;
    // record the sequence of received events, in the form of (event_sender_name, logical_computer_name, event_sequence number)
    map<boost::tuple<string, string>, set<int> > event_received;
...
public:
    ...
    void addSentEvent(string senderName, string LCName, int sequence, simtime_t sent);
    void addReceivedEvent(string senderName, string LCName, int sequence);
};
