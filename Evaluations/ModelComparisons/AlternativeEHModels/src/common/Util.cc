...
string util::convertMapToStr(
        map<boost::tuple<unsigned long, int>, boost::tuple<string, simtime_t> >& m) {
    stringstream ss;
    for (map<boost::tuple<unsigned long, int>, boost::tuple<string, simtime_t> >::iterator it =
            m.begin(); it != m.end(); it++) {
        boost::tuple<unsigned long, int> first = it->first;
        boost::tuple<string, simtime_t> second = it->second;
        ss << get<0>(first) << "," << get<1>(first) << "," << get<0>(second)
                << "," << get<1>(second).dbl() << ";";
    }
    string content = ss.str();
    return content;
...
string util::convertMapToStr(
        map<int, boost::tuple<unsigned long, int, string> >& m) {
    stringstream ss;
...
void util::convertStrToMap(string& str,
    ...
    for (size_t i = 0; i < L1Tokens.size(); i++) {
        ...
        vector<string> elements;
        splitString(token, ",", elements);
        unsigned long senderId = strToLong(elements[0]);
        int seq = strToInt(elements[1]);
        string content = elements[2];
        simtime_t arrival = simtime_t(strToDouble(elements[3]));
        boost::tuple<unsigned long, int> key(senderId, seq);
        boost::tuple<string, simtime_t> value(content, arrival);
        m[key] = value;
    }
}
...
void util::convertStrToMap(string& str,
    ...
    for (size_t i = 0; i < L1Tokens.size(); i++) {
        ...
        vector<string> elements;
        splitString(token, ",", elements);
        int eventIndex = strToInt(elements[0]);
        unsigned long senderId = strToLong(elements[1]);
        int seq = strToInt(elements[2]);
        string content = elements[3];
        boost::tuple<unsigned long, int, string> value(senderId, seq, content);
        m[eventIndex] = value;
    }
}
