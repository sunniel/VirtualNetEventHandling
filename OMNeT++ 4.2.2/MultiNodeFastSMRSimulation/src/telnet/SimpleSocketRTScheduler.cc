...
Register_Class(SimpleSocketRTScheduler);

Register_GlobalConfigOption(CFGID_SIMPLESOCKETRTSCHEDULER_PORT, "socketrtscheduler-port", CFG_INT, "4242", "When cSocketRTScheduler is selected as scheduler class: the port number cSocketRTScheduler listens on.");
...
SimpleSocketRTScheduler::SimpleSocketRTScheduler() : cScheduler()
{
    listenerSocket = INVALID_SOCKET;
    connSocket = INVALID_SOCKET;
}

SimpleSocketRTScheduler::~SimpleSocketRTScheduler()
{
}

void SimpleSocketRTScheduler::startRun()
{
    if (initsocketlibonce()!=0)
        throw cRuntimeError("SimpleSocketRTScheduler: Cannot initialize socket library");
    ...
    port = ev.getConfig()->getAsInt(CFGID_SIMPLESOCKETRTSCHEDULER_PORT);
    setupListener();
}

void SimpleSocketRTScheduler::setupListener()
{
...
void SimpleSocketRTScheduler::endRun()
{
    if(connSocket != INVALID_SOCKET){
        // for windows socket close
        closesocket(connSocket);
        // for *nix socket close
        close(connSocket);
        connSocket = INVALID_SOCKET;
    }
    if(listenerSocket != INVALID_SOCKET){
...
void SimpleSocketRTScheduler::executionResumed()
{
    gettimeofday(&baseTime, NULL);
    baseTime = timeval_substract(baseTime, SIMTIME_DBL(simTime()));
}

void SimpleSocketRTScheduler::setInterfaceModule(cModule *mod, cMessage *notifMsg, char *buf, int bufSize, int *nBytesPtr)
{
    if (module)
...
bool SimpleSocketRTScheduler::receiveWithTimeout(long usec)
{
...
int SimpleSocketRTScheduler::receiveUntil(const timeval& targetTime)
{
...
cMessage *SimpleSocketRTScheduler::getNextEvent()
{
...
void SimpleSocketRTScheduler::sendBytes(const char *buf, size_t numBytes)
{
    if (connSocket==INVALID_SOCKET)
...
bool SimpleSocketRTScheduler::initialized(){
    return module != NULL;
}
