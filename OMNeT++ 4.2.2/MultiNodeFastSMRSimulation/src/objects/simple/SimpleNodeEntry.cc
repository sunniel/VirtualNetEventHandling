...
//    nodeRecord->coords[1] = uniform(0, fieldSize) - fieldSize / 2;
    ...
    UdpIPv4ingate = node->gateHalf("tcpgate", cGate::INPUT);
}
...
//        const SimpleNodeEntry& dest, bool faultyDelay) {
//    if ((pow(1 - tx.errorRate, msg->getByteLength() * 8) <= uniform(0, 1))
//            || (pow(1 - dest.rx.errorRate, msg->getByteLength() * 8)
//                    <= uniform(0, 1))) {
...
//                << "\n    tx.maxQueueTime = " << tx.maxQueueTime << endl;
//        return SimpleDelay(0, false);
//    }
//
//    tx.finished = newTxFinished;
//
//    simtime_t destBandwidthDelay = (msg->getByteLength() * 8)
//            / dest.rx.bandwidth;
...
//
//    return SimpleDelay(
//            tx.finished - now + tx.accessDelay + coordDelay + destBandwidthDelay
//                    + dest.rx.accessDelay, true);
//}
//
//SimpleNodeEntry::SimpleDelay SimpleNodeEntry::calcAccessRouterDelay(
//        cPacket* msg) {
//    simtime_t now = simTime();
//    simtime_t bandwidthDelay = ((msg->getByteLength() * 8) / rx.bandwidth);
//    simtime_t newRxFinished = std::max(
//            rx.finished + rx.accessDelay + bandwidthDelay, now);
//
...
//                << "\n    rx.maxQueueTime = " << rx.maxQueueTime << endl;
//        return SimpleDelay(0, false);
//    }
//
