...
string util::convertMapToStr(map<tuple<int, unsigned long, int> , string>& m) {
    stringstream ss;
    for (map<tuple<int, unsigned long, int> , string>::iterator it = m.begin();
            it != m.end(); it++) {
        tuple<int, unsigned long, int> first = it->first;
...
void util::convertStrToMap(string& str,
        map<tuple<int, unsigned long, int> , string>& m) {
    // empty map first;
    m.clear();
    vector<string> L1Tokens;
    splitString(str, ";", L1Tokens);
    for (size_t i = 0; i < L1Tokens.size(); i++) {
        ...
        tuple<int, unsigned long, int> key(round, senderId, seq);
        m[key] = elements[3];
    }
}
...
string util::packSenderSeqRanges(
        map<unsigned long, tuple<int, int> >& senders) {
    stringstream ss;
    for (map<unsigned long, tuple<int, int> >::iterator it = senders.begin();
            it != senders.end(); it++) {
        unsigned long senderId = it->first;
        tuple<int, int> range = it->second;
...
void util::unpackSenderSeqRanges(string& senderInfo,
    ...
    for (size_t i = 0; i < senderInfos.size(); i++) {
        ...
        tuple<int, int> seqRange(lowestSeq, sor);
