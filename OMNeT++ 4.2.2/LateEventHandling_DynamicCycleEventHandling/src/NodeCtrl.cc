...
//    scheduleAt(cycleStart, updateTimeout);
    ...
    signal_consensus_counter = registerSignal("consensus");
    consensus_counter = 0;
...
void NodeCtrl::finish() {
    emit(signal_consensus_counter, consensus_counter);
}
...
void NodeCtrl::initNeighbors() {
    ...
    // init neighbor list
    for (cModule::SubmoduleIterator iter(getParentModule()->getParentModule());
            !iter.end(); iter++) {
        cModule* module = iter();
    ...
    for (cModule::SubmoduleIterator iter(getNetwork()); !iter.end(); iter++) {
        cModule* module = iter();
...
string NodeCtrl::roundEToString(int round, string senderSeqRanges) {
    ...
    EV << "[" << simTime() << "s] " << fullName << "senderSeqRanges: "
              << senderSeqRanges << endl;
...
//            int r = rnd - (sor - j);
            for (size_t i = 0; i < neighborList.size(); i++) {
                ...
                if (Decisions.count(dKey) == 0
                    ...
                    // compare the event EMPTY_EVENT for late (but still deliverable) events
                    if (event.compare(event::MISS_EVENT) != 0
                            && event.compare(event::EMPTY_EVENT) != 0) {
                        Decisions[dKey] = event;

                        EV << "[Debug] non-missing and non-empty event = "
                                  << event << endl;

                    } else {
                        Decisions[dKey] = event::EMPTY_EVENT;
                        EV << "[Debug] missing or empty event = " << event
                                  << endl;
                    }
                } else {
...
void NodeCtrl::disseminateEvent() {
    ...
    for (map<unsigned long, int>::iterator it = Nr.begin(); it != Nr.end();
        ...
        for (map<int, string>::iterator it3 = addresses.begin();
...
void NodeCtrl::handleGossip(Gossip* gossip) {
    ...
    if (includes(histories | map_keys, survivedMembers | map_keys)) {
        ...
        // sender ID, sequence no
        tuple<unsigned long, int> tuple_last_seq;

        size_t trail_empty = 0;
        vector<int> purge_qc;
        vector<int> purge_qa;
        ...
        int lastRemove =
                lastCommRound <= lastDeliveredRound ?
                        lastCommRound : lastDeliveredRound;
        // retain trailing rounds with Empty events in Qd
        for (map<tuple_qd, string>::reverse_iterator it = Qd.rbegin();
                it != Qd.rend(); it++) {
            tuple_qd key = it->first;
            int r = get<0>(key);
            if (r == lastRemove && Qd[key].compare(event::EMPTY_EVENT) == 0) {
                if (lastRemove > 0) {
                    lastRemove--;
                }
            } else if (r < lastRemove) {
                break;
            }
        }
...
//        for(map<tuple_qd, string>::reverse_iterator it = Qd.rbegin(); it != Qd.rend(); it++){
//            tuple_qd key = it->first;
//            if(Qd[key].compare(event::EMPTY_EVENT) == 0){
//                trail_empty++;
//            }
//            else{
//                break;
...
//        }
        ...
        size_t last = 0;
        if (purge_qd.size() > trail_empty) {
            last = purge_qd.size() - trail_empty;
        }
        for (size_t i = 0; i < last; i++) {
            Qd.erase(purge_qd[i]);
            // purge Qp
            tuple_qd key = purge_qd[i];
            int lastRound = get<0>(key);
            unsigned long sender = get<1>(key);
            unsigned long seq = get<2>(key);
            for(int i = 0; i <= lastRound; i++){
                tuple_qp key_qp_rm(i, sender, seq);
                Qp.erase(key_qp_rm);
            }
