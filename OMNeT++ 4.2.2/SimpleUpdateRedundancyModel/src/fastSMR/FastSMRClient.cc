...
// buffer sent records for result analysis
        ...
        for (map<int, string>::iterator it = updates.begin();
            ...
            if (trace_buffer.find(index) != trace_buffer.end()) {
                trace_buffer[index][content]++;
            } else {
                trace_buffer[index][content] = 1;
            }
            if (trace_buffer[index][content] >= quorum) {
                trace[index] = content;
                trace_buffer.erase(index);
            }
        }
