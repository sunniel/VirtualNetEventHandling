...
#include "BasicSMRNode.h"

Define_Module(BasicSMRNode);

BasicSMRNode::BasicSMRNode() {
    failure = NULL;
    purgerInit = NULL;
    esc = 0;
    escId = 0;
...
//    WATCH_MAP(proposals);
    WATCH_MAP(decisions);
    WATCH_VECTOR(agreements);
    WATCH(esc);
    WATCH(escId);
}

void BasicSMRNode::dispatchHandler(cMessage *msg) {
...
//                      << event->getDestName() << " has been received" << endl;
            ...
            BSMRAgree* agreement = check_and_cast<BSMRAgree*>(msg);
            collectAgreement(agreement);
        } else if (msg->isName(msg::FINAL)) {
            BSMRFinal* final = check_and_cast<BSMRFinal*>(msg);
            handleFinal(final);
...
void BasicSMRNode::initNeighbors() {
    ...
    for (cModule::SubmoduleIterator iter(getParentModule()); !iter.end();
            iter++) {
        cModule* module = iter();
        if (dynamic_cast<BasicSMRNode*>(module) && this != module) {
...
void BasicSMRNode::handleEvent(Event* event) {
    Qr[event->getContent()] = simTime();
    // delete the received msg
    delete event;
}

void BasicSMRNode::startPropose() {
    if (stage == EH && idle) {
        idle = false;
...
void BasicSMRNode::collectProposal(BSMRProposal* proposal) {
...
void BasicSMRNode::handleDecision(BSMRDecision* decision) {
    // validate message
    ...
    if (e == epoch && gid == groupId && stage == EH) {
        ...
        BSMRAgree* agreement = new BSMRAgree(msg::AGREEMENT);
...
void BasicSMRNode::collectAgreement(BSMRAgree* agreement) {
    ...
    // message validation, TODO the first two conditions may not needed
    if (e == epoch && gid == groupId && stage == EH) {
        agreements.push_back(senderName);
...
void BasicSMRNode::handleFinal(BSMRFinal* final) {
    // validate message
...
void BasicSMRNode::contentToEvents(string content,
        map<string, simtime_t>& events) {
...
string BasicSMRNode::QrToString() {
    string events;
    stringstream ss;
...
void BasicSMRNode::handleHeartbeat(HBProbe *hb) {
...
void BasicSMRNode::stageTransfer() {
...
void BasicSMRNode::handleCCQuery(CCQuery* cc) {
...
void BasicSMRNode::handleGRQuery(GRQuery* gr) {
...
void BasicSMRNode::handleCCNAck(CCNAck* ccnack) {
...
void BasicSMRNode::handleCCQueryReply(CCQueryReply* ccr) {
...
void BasicSMRNode::handleGRQueryReply(GRQueryReply* grr) {
...
void BasicSMRNode::handleCCEnd(CCEnd* cce) {
...
void BasicSMRNode::handleGREnd(GREnd* gre) {
...
void BasicSMRNode::checkLastStates() {
...
void BasicSMRNode::checkReadies() {
    if (stage != CC) {
        bool allReceived = true;
...
void BasicSMRNode::checkProposals() {
    if (stage == EH) {
        bool allReceived = true;
        map<string, int> survivedNeighbors = util::intersectMaps(survivals,
                neighbors);
...
void BasicSMRNode::checkAgreements() {
    if (stage == EH) {
        bool allReceived = true;
        map<string, int> survivedNeighbors = util::intersectMaps(survivals,
                neighbors);
        ...
        for (map<string, int>::iterator it = survivedNeighbors.begin();
                it != survivedNeighbors.end(); ++it) {
            string hostName = it->first;
            if (find(agreements.begin(), agreements.end(), hostName)
                    == agreements.end()) {
                allReceived = false;
                break;
            }
        ...
        if (allReceived) {
            ...
            BSMRFinal* final = new BSMRFinal(msg::FINAL);
            ...
            // fake the send-to-self process
            final = check_and_cast<BSMRFinal*>(msg);
            handleFinal(final);
        }
    }
}

string BasicSMRNode::buildStateForSync() {
...
LastState BasicSMRNode::syncState(map<string, LastState>& lsMap) {
    LastState ls;
    int largestEid = 0;
...
Ready BasicSMRNode::syncState(map<string, Ready>& readyMap) {
    Ready ready;
    int largestEid = 0;
...
string BasicSMRNode::coordinator() {
...
LastState BasicSMRNode::computeLastState() {
...
Ready BasicSMRNode::computeReady() {
...
cMessage* BasicSMRNode::broadcast(cMessage* message, map<string, int>& hosts) {
    GatebasedModule::multicast(message, hosts);
    return message;
}

void BasicSMRNode::fail() {
    state = Off;
    par("state") = state;
}
