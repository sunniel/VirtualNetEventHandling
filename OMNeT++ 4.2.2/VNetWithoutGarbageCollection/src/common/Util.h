...
string convertMapToStr(map<tuple<int, unsigned long, int>, string>& m);
...
void convertStrToMap(string& str, map<tuple<int, unsigned long, int>, string>& m);

// num indicates the number variable arguments, and variable arguments can only be const char*
vector<string> loadMapKey(map<string, int>& m, int num...);

string packSenderSeqRanges(map<unsigned long, tuple<int, int> >& senders);
void unpackSenderSeqRanges(string& senderInfo, map<unsigned long, tuple<int, int> >& senders);
