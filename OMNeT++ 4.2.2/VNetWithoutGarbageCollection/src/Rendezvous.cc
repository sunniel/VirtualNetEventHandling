...
void Rendezvous::addNodes(int num, bool isInit) {
    ...
    const char* hostType = par("hostType");
    const char* hostName = par("hostName");
//    const char* hostModule = par("hostModule");

    for (int i = 0; i < num; i++) {
        // create a new node
        cModuleType *moduleType = cModuleType::get(hostType);
        // create (possibly compound) module and build its submodules (if any)
        cModule *host = moduleType->create(hostName, parent, hostCreated + 1,
                hostCreated);
