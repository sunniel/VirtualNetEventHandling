...
public final class PropertiesLoader {
	 ...
	 */
	public final static String STANDARD_PROPERTY_FILE = "application.properties";

	/**
	 * Reads the properties from property file. Either uses the standard property
	 * file or, if specified, a given property file. In either case the property
	 * file must be located in the classpath.
	 */
	public static void loadPropertyFile() {
		...
		try {
			...
			InputStream input = classLoader.getResourceAsStream(STANDARD_PROPERTY_FILE);
			Properties props = new Properties();
			props.load(input);
			AppContext.init(props);
		} catch (IOException e) {
			throw new RuntimeException("Property file was not found: " + STANDARD_PROPERTY_FILE
					+ "! It must be located in the CLASSPATH and " + "be named 'application.properties'", e);
