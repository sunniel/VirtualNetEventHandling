...
#include "BasicSMRRend.h"

Define_Module(BasicSMRRend);

BasicSMRRend::BasicSMRRend() {
    hostCreated = 0;
    init = NULL;
    hbInit = NULL;
}
...
BasicSMRRend::~BasicSMRRend() {
    ...
    if(hbInit != NULL){
        cancelAndDelete(hbInit);
    }
}

void BasicSMRRend::initialize() {
...
void BasicSMRRend::dispatchHandler(cMessage *msg) {
...
void BasicSMRRend::handleHeartbeatResp(HBResponse* hbr) {
...
void BasicSMRRend::handleTimeout(HBTimeout* timeout) {
...
void BasicSMRRend::handleHeartbeatInit(cMessage* msg) {
...
void BasicSMRRend::addNodes(int num, bool isInit) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        cModuleType *moduleType = cModuleType::get(hostType.c_str());
...
void BasicSMRRend::disposeTimeouts() {
