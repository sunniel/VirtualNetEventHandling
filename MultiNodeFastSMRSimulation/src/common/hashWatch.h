...
class SIM_API cHashSetWatcher : public cStdVectorWatcherBase
{
    protected:
        ...
        mutable typename set<T>::iterator it;
        mutable int itPos;
    public:
        cHashSetWatcher(const char *name, set<T>& var) : cStdVectorWatcherBase(name), v(var) {
            itPos=-1;
...
template <class T>
void createHashSetWatcher(const char *varname, set<T>& v)
{
...
{
    protected:
        map<KeyT,ValueT,CmpT>& m;
        mutable typename map<KeyT,ValueT,CmpT>::iterator it;
        mutable int itPos;
        std::string classname;
    public:
        cHashMapWatcher(const char *name, map<KeyT,ValueT,CmpT>& var) : cStdVectorWatcherBase(name), m(var) {
            itPos=-1;
...
template <class KeyT, class ValueT, class CmpT>
void createHashMapWatcher(const char *varname, map<KeyT,ValueT,CmpT>& m)
{
...
{
    protected:
        const map<KeyT,ValueT,CmpT>& m;
        mutable typename map<KeyT,ValueT,CmpT>::const_iterator it;
        mutable int itPos;
        std::string classname;
    public:
        cConstHashMapWatcher(const char *name, const map<KeyT,ValueT,CmpT>& var) : cStdVectorWatcherBase(name), m(var) {
            itPos=-1;
            classname = std::string("unordered_map<")+opp_typename(typeid(KeyT))+","+opp_typename(typeid(ValueT))+">";
        }
        const char *getClassName() const {return classname.c_str();}
...
template <class KeyT, class ValueT, class CmpT>
void createHashMapWatcher(const char *varname, const map<KeyT,ValueT,CmpT>& m)
{
