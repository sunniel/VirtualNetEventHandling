...
//    map<unsigned long, int> recipients;
    ...
    // Rg, msg sending indicators, in tuple (recipient/recipient group ID, recipient(s) address (address, name))
...
protected:
    ...
    virtual void handleHeartbeatTimeout(HBTimeout* hbt);
...
//    virtual void sendReliable(cMessage* msg);
    ...
    virtual set<string> survivedMemberSet();
    virtual vector<string> survivedMembers();
    virtual IPvXAddress loadIPAddress();
